...
class SideMenu(unittest.TestCase):
    ...
    def test_DQS_T16924(self):
        try:
            ...
            phone_input_box.send_keys("01066666666")
            ...
            prifile = self.driver.find_element(AppiumBy.ACCESSIBILITY_ID, "666\n#01066666666")
            prifile.click()
    ...
    def test_DQS_T16925_T16926(self):
        try:
            print("DQS_T16925 프로필에서 비밀번호 재설정 인증번호 입력 페이지의 인증번호 재전송 버튼 기능 동작 확인 || DQS-T16926 프로필에서 비밀번호 재설정 인증번호 입력 페이지의 인증시간 만료 시 기능 동작 확인")
            ...
            phone_input_box = self.driver.find_element(AppiumBy.XPATH, "//android.widget.ImageView[@content-desc=\"휴대폰 번호\n비밀번호\"]/android.widget.EditText[1]")
            phone_input_box.click()
            phone_input_box.send_keys("01077777777")
            ...
            prifile = self.driver.find_element(AppiumBy.ACCESSIBILITY_ID, "777\n#01077777777")
            prifile.click()
            ...
            st2 = self.driver.find_element(AppiumBy.ACCESSIBILITY_ID, "인증완료")
            st2.click()

            st3 = self.driver.find_element(AppiumBy.ACCESSIBILITY_ID, "재전송")
            st3.click()
            ...
            print("DQS_T16925 프로필에서 비밀번호 재설정 인증번호 입력 페이지의 인증번호 재전송 버튼 기능 동작 확인 || DQS-T16926 프로필에서 비밀번호 재설정 인증번호 입력 페이지의 인증시간 만료 시 기능 동작 확인 | Pass")
    ...
    def test_DQS_T16930(self):
        try:
            ...
            phone_input_box = self.driver.find_element(AppiumBy.XPATH, "//android.widget.ImageView[@content-desc=\"휴대폰 번호\n비밀번호\"]/android.widget.EditText[1]")
            phone_input_box.click()
            phone_input_box.send_keys("01088888888")
            ...
            prifile = self.driver.find_element(AppiumBy.ACCESSIBILITY_ID, "888\n#01088888888")
            prifile.click()
            ...
            assert self.driver.find_element(AppiumBy.ACCESSIBILITY_ID, "888").is_displayed()
            assert self.driver.find_element(AppiumBy.ACCESSIBILITY_ID, "휴대폰 번호").is_displayed()
            assert self.driver.find_element(AppiumBy.ACCESSIBILITY_ID, "+82 01088888888").is_displayed()
            assert self.driver.find_element(AppiumBy.ACCESSIBILITY_ID, "device uid").is_displayed()
            assert self.driver.find_element(AppiumBy.ACCESSIBILITY_ID, "32fff773fcf2af14").is_displayed() ##시료폰마다 UID값 다름, 코드 수정 시 확인 필요
